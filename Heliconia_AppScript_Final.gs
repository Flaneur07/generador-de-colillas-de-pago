function doPost(e) {
  try {
    ...
    // Usamos getDisplayValues() para evitar que Google confunda códigos como 8026-1 con fechas
    var values = sheet.getDataRange().getDisplayValues();
    var headers = values[0];
    var headerRowIdx = 0;
    
    // Búsqueda dinámica de encabezados (buscando hacia abajo hasta la fila 15)
    ...
    if (action === 'create') {
      ...
      if (action === 'add_beneficiary' || action === 'delete_beneficiary' || action === 'toggle_beneficiary') {
        ...
        // ¡Secreto aquí! getDisplayValues en vez de getValues para evitar conflictos de Fechas vs Strings (Ej: "8026-1")
        ...
        // Búsqueda del beneficiario: Todo es texto exacto gracias a getDisplayValues()
