function doPost(e) {
  try {
    ...
    for (var i = 0; i < headers.length; i++) {
      ...
      if (colPolizaIdx === -1 && (h === "contrato" || h.includes("contrato") || h.includes("poliza") || h.includes("no."))) {
        colPolizaIdx = i;
      }
      if (colNombreIdx === -1 && (h.includes("nombre") || h.includes("apellido") || h.includes("cliente"))) {
        colNombreIdx = i;
      }
      if (colObsIdx === -1 && (h.includes("obs") || h.includes("nota"))) {
        colObsIdx = i;
      }
    }

    if (colPolizaIdx == -1) return response("Error: Columna 'contrato' no encontrada en la hoja principal.");
    ...
    if (action === 'create') {
      ...
      if (action === 'add_beneficiary' || action === 'delete_beneficiary' || action === 'toggle_beneficiary') {
        ...
        for (var j = 0; j < benHeaders.length; j++) {
          ...
          if (colBenContratoIdx === -1 && (bh.includes("contrato") || bh.includes("póliza") || bh.includes("poliza"))) colBenContratoIdx = j;
          if (colBenNombreIdx === -1 && (bh.includes("nombre") || bh.includes("apellido"))) colBenNombreIdx = j;
          if (colBenFechaIdx === -1 && (bh.includes("nacimiento") || bh.includes("fecha") || bh === "f. nacimiento")) colBenFechaIdx = j;
          if (colBenEstadoIdx === -1 && (bh === "estado" || bh.includes("estado"))) colBenEstadoIdx = j;
        }
